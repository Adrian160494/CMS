    ...
    <div class="col-md-12 manage-panel">
        <div class="col-md-3 sidebar-pages">
            ...
            <div class="pages-content">
                <ul>
                @foreach ($pages as $p)
                        <li class="{{$p->slug == "main_page" ? 'custom-element' :'subpage'}}"><a href="{{url()->route('projekty.manage')}}?id={{$p->id}}&page={{$p->nazwa}}&slug={{$p->slug}}">{{$p->nazwa}}<a style="display: {{$p->slug == "main_page"? 'none':'block'}}" class="delete-page" href="{{url()->route('projekty.manage.deletepage',array('id'=>$p->id))}}"><img src="/img/rubbish-bin.svg" width="20"/> </a></a></li>
                    @endforeach
                </ul>
            </div>
        </div>
